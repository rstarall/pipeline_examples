...
2. **智能筛选搜索结果**：
   - **重要**：首先识别并过滤掉与用户问题无关的搜索结果
   - 只提取与用户问题直接相关的关键实体、概念和事实
   - 识别重要的时间、地点、人物、事件等相关信息
   - 分析搜索结果中与问题相关的关联关系和因果关系
   - 忽略偏离主题的噪音信息和无关内容

3. **构建针对性查询**：
   - 将用户问题、筛选后的相关搜索信息和对话背景有机结合
   - 生成一个包含多个维度和角度的详细查询
   - 确保查询内容紧扣用户问题的核心，避免偏离主题
...
**请直接输出生成的LightRAG查询，不要包含其他解释文字**："""
    ...
    def _stream_response(self, query: str, messages: List[dict]) -> Generator[str, None, None]:
        ...
        try:
            ...
            for chunk in self._emit_processing(f"{lightrag_query[:100]}{'...' if len(lightrag_query) > 100 else ''}", "lightrag_query_generation"):
                yield f"data: {json.dumps(chunk)}\n\n"
