"""
title: COT LightRAG Pipeline
author: open-webui
date: 2024-12-20
version: 1.0
license: MIT
description: 一个双管线CoT系统：管线1直接生成原始回答(对比)，管线2多步骤递进检索(3阶段LightRAG)，两条管线并行执行
...
class Pipeline:
    ...
    def __init__(self):
        self.name = "COT LightRAG Pipeline"
        # 初始化token统计
        self.token_stats = {
            "input_tokens": 0,
            "output_tokens": 0,
    ...
    async def on_startup(self):
        print(f"COT LightRAG Pipeline启动: {__name__}")
    ...
    async def on_shutdown(self):
        print(f"COT LightRAG Pipeline关闭: {__name__}")
    ...
    def _pipeline1_direct_answer(self, optimized_query: str, messages: List[dict]) -> Generator[str, None, None]:
        ...
        try:
            ...
            if isinstance(response, dict) and "error" in response:
                yield f"直接回答生成失败: {response['error']}"
                return
    ...
    def _pipeline2_stage1_retrieval(self, optimized_query: str, messages: List[dict]) -> tuple:
        """管线2阶段1：初始检索"""
        try:
            context = self._get_conversation_context(messages)
            ...
            # 执行LightRAG检索
            lightrag_result = self._query_lightrag(stage1_query)
    ...
    def _pipeline2_stage2_retrieval(self, optimized_query: str, stage1_query: str, stage1_results: str) -> tuple:
        """管线2阶段2：基于第一阶段结果的深度检索"""
        try:
            # 生成第二阶段检索查询
            prompt = STAGE2_QUERY_REFINEMENT_PROMPT.format(
                original_query=optimized_query,
                stage1_query=stage1_query,
                stage1_results=stage1_results  # 移除长度限制，使用完整内容
            )
            ...
            # 执行LightRAG检索
            lightrag_result = self._query_lightrag(stage2_query)
    ...
    def _pipeline2_stage3_retrieval(self, optimized_query: str, stage1_query: str, stage1_results: str, 
                                   stage2_query: str, stage2_results: str) -> tuple:
        """管线2阶段3：综合性检索"""
        try:
            # 生成第三阶段检索查询
            prompt = STAGE3_COMPREHENSIVE_PROMPT.format(
                ...
                stage2_results=stage2_results   # 移除长度限制，使用完整内容
            )
            ...
            # 执行LightRAG检索
            lightrag_result = self._query_lightrag(stage3_query)
    ...
    def _pipeline2_final_synthesis(self, optimized_query: str, stage1_query: str, stage1_results: str,
        ...
        try:
            ...
            prompt = FINAL_ANSWER_GENERATION_PROMPT.format(
                ...
                stage3_results=stage3_results   # 移除长度限制，使用完整内容
            )
    ...
    def _stream_response(self, query: str, messages: List[dict]) -> Generator[str, None, None]:
        ...
        try:
            ...
            # 流式处理管线1的直接回答
            for chunk in self._pipeline1_direct_answer(optimized_query, messages):
                yield chunk
            ...
            stage1_query, stage1_results = self._pipeline2_stage1_retrieval(optimized_query, messages)

            for chunk in self._emit_processing(f"✅ 第一阶段检索完成\n\n**检索查询:**\n{stage1_query}\n\n**检索结果:**\n{stage1_results}", "stage1_retrieval"):
                yield f"data: {json.dumps(chunk)}\n\n"
            ...
            stage2_query, stage2_results = self._pipeline2_stage2_retrieval(optimized_query, stage1_query, stage1_results)

            for chunk in self._emit_processing(f"✅ 第二阶段检索完成\n\n**检索查询:**\n{stage2_query}\n\n**检索结果:**\n{stage2_results}", "stage2_retrieval"):
                yield f"data: {json.dumps(chunk)}\n\n"
            ...
            final_answer = self._pipeline2_final_synthesis(
    ...
    def _non_stream_response(self, query: str, messages: List[dict]) -> str:
        ...
        try:
                    ...
                    try:
                        ...
                        if "choices" in data and data["choices"]:
                            ...
                            if processing_content:
                                direct_answer_parts.append(processing_content)
                    except (json.JSONDecodeError, KeyError):
                        continue
            
            direct_answer = "".join(direct_answer_parts).strip() or "未获取到回答内容"
            ...
            # 构建完整响应
            response_parts = []
            response_parts.append("# COT LightRAG双管线执行结果")
            response_parts.append(f"\n## 🎯 问题优化阶段")
            response_parts.append(f"**原始问题:** {query}")
            response_parts.append(f"**优化后的问题:** {optimized_query}")
            
            response_parts.append(f"\n## 📝 管线1 - 直接回答（对比基准）")
            response_parts.append(f"{direct_answer}")
            
            response_parts.append(f"\n## 🧠 管线2 - 多阶段检索最终答案")
            response_parts.append(f"{final_answer}")
            
            response_parts.append(f"\n## 🔍 详细检索过程")
            response_parts.append(f"\n### 第一阶段检索")
            response_parts.append(f"**检索查询:**\n{stage1_query}")
            response_parts.append(f"\n**检索结果:**\n{stage1_results}")
            ...
            token_stats = self._get_token_stats()
            response_parts.append(f"\n\n---")
            response_parts.append(f"**Token消耗统计**")
            response_parts.append(f"- 输入Token: {token_stats['input_tokens']:,}")
            response_parts.append(f"- 输出Token: {token_stats['output_tokens']:,}")
            response_parts.append(f"- 总Token: {token_stats['total_tokens']:,}")

            # 添加配置信息
            response_parts.append(f"\n**配置信息**")
            response_parts.append(f"- LightRAG模式: {self.valves.LIGHTRAG_DEFAULT_MODE}")
            response_parts.append(f"- 模型: {self.valves.OPENAI_MODEL}")
