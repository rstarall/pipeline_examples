        ...
        for i, paper in enumerate(papers[:20], 1):  # 限制在前20篇
            ...
            if paper.get('urls'):
                ...
                if isinstance(urls, list) and urls:
                    papers_detail += f"  • Semantic Scholar: {urls[0]}\n"
                elif isinstance(urls, str):
                    papers_detail += f"  • Semantic Scholar: {urls}\n"
...
作为资深学术研究专家，请对以下论文进行深入分析并回答用户问题：

### 🔍 内容深度分析和输出要求
1. **摘要精读分析**：仔细解读每篇论文的完整摘要，提取核心研究问题、方法创新、关键发现和结论
2. **研究方法评述**：分析各论文采用的研究方法、实验设计、数据来源的优势与局限性  
3. **关键发现提取**：识别每篇论文的重要数据、统计结果、性能指标、创新突破点
4. **学术价值评估**：基于引用数、影响因子、研究质量评估论文的学术贡献和可信度

### 🧩 综合分析和输出要求  
5. **跨论文比较**：对比不同研究的方法、结果、结论，识别一致性发现和争议分歧
6. **研究趋势洞察**：分析该领域的发展脉络、新兴方向、技术演进趋势
7. **知识综合构建**：将分散的研究发现整合为系统性知识体系和深入见解
8. **批判性思考**：指出研究空白、方法局限、需要进一步验证的观点

### 📚 引用与链接规范和输出要求
9. **完整引用格式**：每个观点必须标注 [论文标题，作者，年份，引用数]
10. **链接优先级**：优先提供开放获取PDF→DOI→ArXiv→PubMed，便于读者深入阅读
11. **证据支撑**：每个结论都要有具体的论文摘要内容和数据支撑，避免空泛表述
12. **链接要求**：使用markdown格式输出，使用[链接](url)格式输出，不要使用<>格式输出

**输出要求**：提供结构化、有深度、有批判性的学术分析报告，而非简单的信息罗列
