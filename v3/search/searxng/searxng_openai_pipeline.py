...
9. 在回答末尾添加"## 参考来源"部分，将引用的来源以markdown链接格式列出，格式为：[标题](链接)
...
请用中文回答，语言要自然流畅。"""
        ...
        if context_text.strip():
            user_prompt = f"""基于以下对话历史和搜索结果回答用户的问题。
...
2. 仅使用与问题相关的信息，忽略广告、导航菜单、版权信息等与主题无关的内容
3. 基于筛选后的有效信息提供准确回答
4. 如果搜索结果不足以完全回答问题，请说明哪些部分需要更多信息
5. 保持回答的结构清晰，使用适当的格式
6. 在回答末尾添加“## 参考来源”部分，将引用的来源以markdown链接格式列出，格式为：[标题](链接)"""
        else:
            user_prompt = f"""基于以下搜索结果回答用户的问题。
...
请根据搜索结果提供准确、详细且有用的回答：
1. 理解用户的真实需求，仅使用与问题相关的信息
2. 忽略广告、导航菜单、版权信息、网站脚注等与主题无关的内容
3. 基于筛选后的有效信息提供准确回答
4. 如果搜索结果不足以完全回答问题，请说明哪些部分需要更多信息
5. 保持回答的结构清晰，使用适当的格式
6. 在回答末尾添加“## 参考来源”部分，将引用的来源以markdown链接格式列出，格式为：[标题](链接)"""
    ...
    def pipe(self, user_message: str, model_id: str, messages: List[dict], body: dict) -> Union[str, Generator, Iterator]:
        ...
        # 阶段1：查询优化
        if stream_mode:
            for chunk in self._emit_processing("正在优化搜索查询...", "query_optimization"):
